...
function ColorText takes player CPlayer returns string
  ...
  // else
    // if playermax != 12 then
      // if (PColor == PLAYER_COLOR_MAROON) then
        // return "Maroon"
...
function ColorPlayer takes string CompC returns playercolor
  ...
  elseif (CompC == "Brown") then
    ...
    // if  playermax != 12 then
	  // loop
		  // if (GetPlayerColor(Player(i)) == PLAYER_COLOR_MAROON) then
			// return PLAYER_COLOR_MAROON
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_NAVY) then
			// return PLAYER_COLOR_NAVY
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_TURQUOISE) then
			// return PLAYER_COLOR_TURQUOISE
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_VIOLET) then
			// return PLAYER_COLOR_VIOLET
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_WHEAT) then
			// return PLAYER_COLOR_WHEAT
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_PEACH) then
			// return PLAYER_COLOR_PEACH
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_MINT) then
			// return PLAYER_COLOR_MINT
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_LAVENDER) then
			// return PLAYER_COLOR_LAVENDER
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_COAL) then
			// return PLAYER_COLOR_COAL
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_SNOW) then
			// return PLAYER_COLOR_SNOW
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_EMERALD) then
			// return PLAYER_COLOR_EMERALD
		  // elseif (GetPlayerColor(Player(i)) == PLAYER_COLOR_PEANUT) then
...
function MapInit takes nothing returns nothing
  ...
  elseif language == "Chinese" then
    call DisplayToAll("|c0000ffff﻿这张地图包含AMAI和智能指挥模块（AMAI_2.6.1_cn-24人-非原生）|r")
    call DisplayToAll("|c0000ffff输入'CMD:Help'查看帮助信息，按 ESC 可打开控制台|r")
    call DisplayToAll("|c0000ffff登陆 https://github.com/SMUnlimited/AMAI 查询更新和其他信息|r")
    call DisplayToAll("|c0000ffff支持魔兽1.24b~1.32（B站搜索 AMAI 2022 获取仅支持1.29以上的原生24人最新版）|r")
    call DisplayToAll("|c0000ffff建议使用魔兽重置版，搭配AMAI_3.1.1（或更高）及淬火食用|r")
    call DisplayToAll("|c0000ffffAMAI作者：AIAndy, Zalamander, DKSlayer, Strategy Master, Pixyy|r")
    call DisplayToAll("|c0000ffff汉化：Dr.Fan、Sheeryiro（武汉大学）-完善：KeamSpring、JZY（NGA：jzylwf）|r")
    call DisplayToAll("|c0000ffff专为非重置版玩家汉化，已同步部分3.1.1改动|r")
    call DisplayToAll("|c0000ffff已知问题：帮助信息未汉化；中途更换语言效果不佳，AI不会鸟德变乌鸦，未完美适配1.32.10，无法适配24种颜色（非原生的问题）|r")
  endif
...
function BuildStartDialog takes nothing returns nothing
  ...
  if GetPlayerController(Player(i)) == MAP_CONTROL_COMPUTER and GetPlayerSlotState(Player(i)) == PLAYER_SLOT_STATE_PLAYING then
    set sdbn_button[sdbn_length] = DialogAddButton(start_dialog, next_page, IMinBJ(48 + 9, 57))
  endif
...
function BuildStartDialogOne takes nothing returns nothing
  ...
  set sdbn_button[sdbn_length] = DialogAddButton(start_dialog_one, previous_page, IMinBJ(48 , 57))
  ...
  if c == 8 then
    if GetPlayerController(Player(i)) == MAP_CONTROL_COMPUTER and GetPlayerSlotState(Player(i)) == PLAYER_SLOT_STATE_PLAYING then
      set sdbn_button[sdbn_length] = DialogAddButton(start_dialog_one, next_page, IMinBJ(48 + b, 57))
    endif
  endif
...
function BuildStartDialogTwo takes nothing returns nothing
  ...
  set sdbn_button[sdbn_length] = DialogAddButton(start_dialog_two, previous_page, IMinBJ(48 , 57))
...
function BuildNumberDialog takes nothing returns nothing
  ...
  loop
    ...
    set i = i + 1
  endloop
  set ndbn_button[8] = DialogAddButton(number_dialog, next_page, IMinBJ(48 + 9, 57))
...
function BuildNumberDialogOne takes nothing returns nothing
  ...
  set ndbn_button[9] = DialogAddButton(number_dialog_one, previous_page, IMinBJ(48 , 57))
  loop
    exitwhen i >= 18
  ...
  set ndbn_button[18] = DialogAddButton(number_dialog_one, next_page, IMinBJ(48 + i - 9, 57))
...
function BuildNumberDialogTwo takes nothing returns nothing
  ...
  set ndbn_button[19] = DialogAddButton(number_dialog_two, previous_page, IMinBJ(48 , 57))
  loop
    exitwhen i >= 29
...
function BuildPlayerDialog takes nothing returns nothing
  ...
  endloop
  if not IsPlayerObserver(Player(i)) and GetPlayerSlotState(Player(i)) == PLAYER_SLOT_STATE_PLAYING then
    set pcbn_button[9] = DialogAddButton(player_dialog, next_page, IMinBJ(48 + 9, 57))
  endif
...
function BuildPlayerDialogOne takes nothing returns nothing
  ...
  set pcbn_button[10] = DialogAddButton(player_dialog_one, previous_page, IMinBJ(48, 57))
  loop
    exitwhen i > c
  ...
  if playermax != 12 then
    if not IsPlayerObserver(Player(i - 2)) and GetPlayerSlotState(Player(i - 2)) == PLAYER_SLOT_STATE_PLAYING then
      set pcbn_button[19] = DialogAddButton(player_dialog_one, next_page, IMinBJ(48 + 9, 57))
    endif
  endif
...
function BuildPlayerDialogTwo takes nothing returns nothing
  ...
  set pcbn_button[20] = DialogAddButton(player_dialog_two, previous_page, IMinBJ(48, 57))
  loop
    exitwhen i > 27
...
function initLanguageEnglish takes nothing returns nothing
  ...
  set you_no_gold = "Sorry, you don't have enough Gold."
  set i_no_gold = "Sorry, I don't have enough Gold."
  set tribute_of_gold = " Gold is Tribute complete."
  set gold_received_thanks = " Gold has been received. Thanks your Gift."
  set you_no_lumber = "Sorry, you don't have enough Lumber."
  set i_no_lumber = "Sorry, I don't have enough Lumber."
  set tribute_of_lumber = " Lumber is Tribute complete."
  set lumber_received_thanks = " Lumber has been received. Thanks your Gift."
  set you_mistyped_tribute = "Sorry, you mistyped your Tribute quantity."
  set specified_player_not_valid = "You have not specified a valid player."
  set command_not_valid = "I do not know that command."
  set all_not_allowed = "ERROR, All is not allowed here."
  set player_id_mistyped = "ERROR, you mistyped the player id."
  set color_word_mistyped = "ERROR, you mistyped the player Color or Command word."
  set not_specified_computer = "You have not specified a computer."
  set next_page = "next page"
  set previous_page = "previous page"
...
function initLanguageDeutsch takes nothing returns nothing
  ...
  set you_no_gold = "Tut mir leid, du hast nicht genug Gold."
  set i_no_gold = "Tut mir leid, ich habe nicht genug Gold."
  set tribute_of_gold = " Gold ist Tribut komplett."
  set gold_received_thanks = " Gold ist eingegangen. Danke, dein Geschenk."
  set you_no_lumber = "Tut mir leid, du hast nicht genug Holz."
  set i_no_lumber = "Sorry, Tut mir leid, ich habe nicht genug Holz."
  set tribute_of_lumber = " Lumber ist Tribute komplett."
  set lumber_received_thanks = " Bauholz ist eingegangen. Danke, dein Geschenk."
  set you_mistyped_tribute = "Entschuldigung, Sie haben sich bei Ihrer Tribute-Menge vertippt."
  set specified_player_not_valid = "Sie haben keinen gultigen Spieler angegeben."
  set command_not_valid = "Ich kenne diesen Befehl nicht."
  set all_not_allowed = "FEHLER, Alles ist hier nicht erlaubt."
  set player_id_mistyped = "FEHLER, du hast die Spieler-ID falsch eingegeben."
  set color_word_mistyped = "FEHLER, Sie haben die Spielerfarbe oder das Befehlswort falsch eingegeben."
  set not_specified_computer = "Sie haben keinen Computer angegeben."
  set next_page = "Nachste Seite"
  set previous_page = "vorherige Seite"
  set dlghdr_choose_ally = "Wahlen Sie Verbundete"
  set dlghdr_choose_number = "Zahl wahlen"
  set dlghdr_choose_player = "Wahle einen Spieler"
  set dlghdr_game_type = "Wahle den Spielmodus"
  set dlghdr_root = "Befehl wahlen"
  set dlghdr_tribute_type = "Wahle die Tributart"
  set dlghdr_tribute_amount = "Wahle die Tributmenge"
...
function initLanguageSwedish takes nothing returns nothing
  ...
  set you_no_gold = "Tyvarr, du har inte tillrackligt med guld."
  set i_no_gold = "Tyvarr, jag har inte tillrackligt med guld."
  set tribute_of_gold = " Guld ar Tribute komplett."
  set gold_received_thanks = " Guld har tagits emot. Tack din gava."
  set you_no_lumber = "Tyvarr, du har inte tillrackligt med timmer."
  set i_no_lumber = "Tyvarr, jag har inte tillrackligt med timmer."
  set tribute_of_lumber = " Timmer is Tribute komplett."
  set lumber_received_thanks = " Timmer har tagits emot. Tack din gava."
  set you_mistyped_tribute = "Ursakta, du skrev fel i din Tribute-mangd."
  set specified_player_not_valid = "Du har inte angett en giltig spelare."
  set command_not_valid = "Jag kanner inte till det kommandot."
  set all_not_allowed = "FEL, allt ar inte tillatet har."
  set player_id_mistyped = "FEL, du skrev fel spelar-id."
  set color_word_mistyped = "FEL, du skrev fel i spelarens farg eller kommandoord."
  set not_specified_computer = "Du har inte angett en dator."
  set next_page = "nasta sida"
  set previous_page = "foregaende sida"
  set dlghdr_choose_ally = "Valj din lierade"
  set dlghdr_choose_number = "Valj nummer"
  set dlghdr_choose_player = "Valj spelare"
  set dlghdr_game_type = "Valj speltyp"
  set dlghdr_root = "Valj kommando"
  set dlghdr_tribute_type = "Valj bidragsmetod"
  set dlghdr_tribute_amount = "Valj bidragsstorlek"
...
function initLanguageFrench takes nothing returns nothing
  set chat_no_ally = "Je ne t'obeirai jamais."
  set you_no_gold = "Desole, vous n'avez pas assez d'or."
  set i_no_gold = "Desole, je n'ai pas assez d'or."
  set tribute_of_gold = " L'or est Tribute complet."
  set gold_received_thanks = " L'or a ete recu. Merci votre cadeau."
  set you_no_lumber = "Desole, vous n'avez pas assez de bois."
  set i_no_lumber = "Desole, je n'ai pas assez de bois."
  set tribute_of_lumber = " Le bois est Tribute complet."
  set lumber_received_thanks = " Le bois a ete receptionne. Merci votre cadeau."
  set you_mistyped_tribute = "Desole, vous avez mal tape votre quantite d'hommage."
  set specified_player_not_valid = "Vous n'avez pas specifie de joueur valide."
  set command_not_valid = "Je ne connais pas cette commande."
  set all_not_allowed = "ERREUR, Tout n'est pas autorisÃ© ici."
  set player_id_mistyped = "ERREUR, vous avez mal saisi l'identifiant du joueur."
  set color_word_mistyped = "ERREUR, vous avez mal tape la couleur du joueur ou le mot de commande."
  set not_specified_computer = "Vous n'avez pas specifie d'ordinateur."
  set next_page = "page suivante"
  set previous_page = "page precedente"
  ...
  set dlgbutton_give_gold = "donner du or"
  set dlgbutton_give_lumber = "donner du bois"
...
function initLanguageSpanish takes nothing returns nothing
  ...
  set you_no_gold = "Lo sentimos, no tienes suficiente oro."
  set i_no_gold = "Lo siento, no tengo suficiente oro."
  set tribute_of_gold = " El oro es Tributo completo."
  set gold_received_thanks = " Se ha recibido oro. Gracias tu regalo."
  set you_no_lumber = "Lo siento, no tienes suficiente madera."
  set i_no_lumber = "Lo siento, no tengo suficiente madera."
  set tribute_of_lumber = " La madera es tributo completo."
  set lumber_received_thanks = " Se ha recibido madera. Gracias tu regalo."
  set you_mistyped_tribute = "Lo sentimos, escribiste mal la cantidad de tu Tributo."
  set specified_player_not_valid = "No ha especificado un jugador valido."
  set command_not_valid = "No conozco ese comando."
  set all_not_allowed = "ERROR, No todo esta permitido aqui."
  set player_id_mistyped = "ERROR, escribiste mal la identificacion del jugador."
  set color_word_mistyped = "ERROR, escribiste mal la palabra Color o Comando del jugador."
  set not_specified_computer = "No ha especificado una computadora."
  set next_page = "siguiente pagina"
  set previous_page = "pagina anterior"
...
function initLanguageRomanian takes nothing returns nothing
  ...
  set you_no_gold = "Ne pare rau, nu aveti suficient Aur."
  set i_no_gold = "Imi pare rau, nu am suficient Aur."
  set tribute_of_gold = " Aurul este un tribut complet."
  set gold_received_thanks = " Aurul a fost primit. MulČumesc cadou."
  set you_no_lumber = "Imi pare rau, nu aveti suficienta cherestea."
  set i_no_lumber = "Sorry, imi pare rau, nu am destula cherestea."
  set tribute_of_lumber = " Cherestea este un tribut complet."
  set lumber_received_thanks = " Primit cherestea. MulČumesc cadou."
  set you_mistyped_tribute = "Ne pare rau, ati introdus gresit cantitatea Tribute."
  set specified_player_not_valid = "Nu ati specificat un jucator valid."
  set command_not_valid = "Nu cunosc comanda aia."
  set all_not_allowed = "EROARE, Totul nu este permis aici."
  set player_id_mistyped = "EROARE, ati introdus gresit id-ul jucatorului."
  set color_word_mistyped = "EROARE, ati introdus gresit cuvantul Culoare sau Comanda."
  set not_specified_computer = "Nu ati specificat un computer."
  set next_page = "pagina urmatoare"
  set previous_page = "pagina anterioara"
...
function initLanguageRussian takes nothing returns nothing
  ...
  set you_no_gold = "Извините, у вас недостаточно золота."
  set i_no_gold = "Извините, у меня недостаточно золота."
  set tribute_of_gold = " Золото завершено."
  set gold_received_thanks = " Золото получено. Спасибо твой Дар."
  set you_no_lumber = "Извините, у вас недостаточно пиломатериалов."
  set i_no_lumber = "Извините, у меня недостаточно пиломатериалов."
  set tribute_of_lumber = " Пиломатериалы завершены."
  set lumber_received_thanks = " Получен пиломатериал. Спасибо твой Дар."
  set you_mistyped_tribute = "Извините, вы неправильно указали количество Tribute."
  set specified_player_not_valid = "Вы не указали действительного игрока."
  set command_not_valid = "Я не знаю этой команды."
  set all_not_allowed = "ОШИБКА, здесь нельзя все."
  set player_id_mistyped = "ОШИБКА, вы ошиблись при вводе идентификатора игрока."
  set color_word_mistyped = "ОШИБКА, вы неправильно набрали цвет игрока или командное слово."
  set not_specified_computer = "Вы не указали компьютер."
  set next_page = "следущая страница"
  set previous_page = "Предыдущая страница"
...
function initLanguagePortuguese takes nothing returns nothing
  ...
  set you_no_gold = "Desculpe, voce nao tem ouro suficiente."
  set i_no_gold = "Desculpe, nao tenho ouro suficiente."
  set tribute_of_gold = " Ouro e Tributo completo."
  set gold_received_thanks = " Ouro foi recebido. Obrigado seu presente."
  set you_no_lumber = "Desculpe, voce nao tem madeira suficiente."
  set i_no_lumber = "Desculpe, nao tenho madeira suficiente."
  set tribute_of_lumber = " Madeira serrada e Tributo completo."
  set lumber_received_thanks = " madeira foi recebida. Obrigado seu presente."
  set you_mistyped_tribute = "Desculpe, voce digitou incorretamente a quantidade de Tributos."
  set specified_player_not_valid = "Voce nao especificou um jogador valido."
  set command_not_valid = "Nao conheco esse comando."
  set all_not_allowed = "ERRO, Tudo nao e permitido aqui."
  set player_id_mistyped = "ERRO, voce digitou incorretamente o ID do jogador."
  set color_word_mistyped = "ERRO, voce digitou incorretamente a palavra Cor ou Comando do jogador."
  set not_specified_computer = "Voce nao especificou um computador."
  set next_page = "proxima pagina"
  set previous_page = "pagina anterior"
  set dlghdr_choose_ally = "Aliado Escolhido"
  set dlghdr_choose_number = "Escolha numero"
...
function initLanguageNorwegian takes nothing returns nothing
  set chat_no_ally = "Jeg vil aldri adlyde deg!"
  set you_no_gold = "Beklager, du har ikke nok gull."
  set i_no_gold = "Beklager, jeg har ikke nok gull."
  set tribute_of_gold = " Gold is Tribute komplett."
  set gold_received_thanks = " Gull er mottatt. Takk din gave."
  set you_no_lumber = "Beklager, du har ikke nok trelast."
  set i_no_lumber = "Beklager, jeg har ikke nok trelast."
  set tribute_of_lumber = " Trelast is Tribute komplett."
  set lumber_received_thanks = " Trelast er mottatt. Takk din gave."
  set you_mistyped_tribute = "Beklager, du har skrevet feil i Tribute-antallet."
  set specified_player_not_valid = "Du har ikke spesifisert en gyldig spiller."
  set command_not_valid = "Jeg kjenner ikke den kommandoen."
  set all_not_allowed = "FEIL, alt er ikke tillatt her."
  set player_id_mistyped = "FEIL, du skrev feil spiller-ID."
  set color_word_mistyped = "FEIL, du skrev feil i spillerens farge eller kommandoord."
  set not_specified_computer = "Du har ikke spesifisert en datamaskin."
  set next_page = "neste side"
  set previous_page = "forrige side"
  ...
  call SaveStr(translation, StringHash(language), StringHash("Chatter"), "Chat")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Computer Chatter."), "Skru av / pa computer chatting.")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Debug Mode."), "Slatt feilsokingsmodus pa / av.")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Balancing Mode."), "Skru av / pa balanserings modus")
  ...
  call SaveStr(translation, StringHash(language), StringHash("Queue"), "Ko")
  call SaveStr(translation, StringHash(language), StringHash("I have Canceled all Queued commands you have given me so far."), "Jeg har kansellert alle kokommandoer du har gitt meg sa langt.")
  call SaveStr(translation, StringHash(language), StringHash("I will Stop any current Attack."), "Jeg vil stoppe ethvert angrep.")
  call SaveStr(translation, StringHash(language), StringHash("I will not Attack Anything."), "Jeg vil ikke angripe noe.")
  call SaveStr(translation, StringHash(language), StringHash("I will not Attack Creeps."), "Jeg vil ikke angripe Creeps.")
  call SaveStr(translation, StringHash(language), StringHash("I will not Attack Enemies."), "Jeg vil ikke angripe fiender.")
  call SaveStr(translation, StringHash(language), StringHash("I will build Air2Air units now."), "Jeg skal bygge Air2Air-enheter na.")
  call SaveStr(translation, StringHash(language), StringHash("I will build Air2Ground units now."), "Jeg skal bygge Air2Ground-enheter na.")
  call SaveStr(translation, StringHash(language), StringHash("I will build Ground2Air units now."), "Jeg skal bygge Ground2Air-enheter na.")
  call SaveStr(translation, StringHash(language), StringHash("I will build Ground2Ground units now."), "Jeg skal bygge Ground2Ground-enheter na.")
  call SaveStr(translation, StringHash(language), StringHash("I will build farms now."), "Jeg skal bygge garder na.")
  call SaveStr(translation, StringHash(language), StringHash("I will build towers now."), "eg skal bygge tarn na.")
  call SaveStr(translation, StringHash(language), StringHash("I will Attack/Guard the location of current screen position."), "Jeg vil angripe / bevokte plasseringen av gjeldende skjermposisjon.")
  call SaveStr(translation, StringHash(language), StringHash("I will Attack/Guard the currently selected unit."), "Jeg vil angripe / bevokte den valgte enheten.")
  call SaveStr(translation, StringHash(language), StringHash("I will Attack/Guard the specified player."), "Jeg vil angripe / bevokte den angitte spilleren.")
  call SaveStr(translation, StringHash(language), StringHash("I have Started or Restarted your Queued commands."), "Jeg har startet eller omstartet kommandoene dine i ko.")
  call SaveStr(translation, StringHash(language), StringHash("I have Paused your Queued commands."), "Jeg har satt kommandoene dine i ko pa pause.")
  call SaveStr(translation, StringHash(language), StringHash("I have Unpaused your Queued commands."), "Jeg har gjenopptatt kommandoene dine i ko.")
  call SaveStr(translation, StringHash(language), StringHash("I have Removed your last Queued command."), "Jeg har fjernet den siste kommandoen din i ko.")
  call SaveStr(translation, StringHash(language), StringHash("I have Queued the Attack/Guard location of current screen position."), "Jeg har satt angreps- / vaktposisjonen til gjeldende skjermposisjon i ko.")
  call SaveStr(translation, StringHash(language), StringHash("I have Queued the currently selected unit to Attack/Guard."), "Jeg har satt den valgte enheten i ko til angrep / vakt.")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Automatic Strategy Change."), "Sla pa / av automatisk strategiendring.")
  call SaveStr(translation, StringHash(language), StringHash("Restrict"), "Begrense")
  call SaveStr(translation, StringHash(language), StringHash("No Attacks"), "Ingen angrep")
  call SaveStr(translation, StringHash(language), StringHash("No Creep Attacks"), "Ingen krypende angrep")
  call SaveStr(translation, StringHash(language), StringHash("No Enemy Attacks"), "Ingen fiendtlige angrep")
  call SaveStr(translation, StringHash(language), StringHash("Farms"), "Garder")
  call SaveStr(translation, StringHash(language), StringHash("Towers"), "Tarn")
  call SaveStr(translation, StringHash(language), StringHash("Current screen"), "Gjeldende skjerm")
  call SaveStr(translation, StringHash(language), StringHash("Selected unit"), "Valgt enhet")
  call SaveStr(translation, StringHash(language), StringHash("Start/Restart"), "Start / start pa nytt")
  call SaveStr(translation, StringHash(language), StringHash("No Strategy Change"), "Ingen strategiendring")
  call SaveStr(translation, StringHash(language), StringHash("Build"), "Bygge")
  call SaveStr(translation, StringHash(language), StringHash("Attack Here"), "Angrip her")
  call SaveStr(translation, StringHash(language), StringHash("Unpause"), "Gjenoppta pause")
  call SaveStr(translation, StringHash(language), StringHash("Undo"), "Angre")
  call SaveStr(translation, StringHash(language), StringHash("attack select"), "angrep velge")
  call SaveStr(translation, StringHash(language), StringHash("Air vs. Air"), "Air2Air")
  call SaveStr(translation, StringHash(language), StringHash("Air vs. Ground"), "Air2Ground")
  call SaveStr(translation, StringHash(language), StringHash("Ground vs. Air"), "Ground2Air")
  call SaveStr(translation, StringHash(language), StringHash("Ground vs. Ground"), "Ground2Ground")
endfunction
function initLanguageChinese takes nothing returns nothing
  set chat_no_ally = "我们不是盟友。"
  set you_no_gold = "翻车了，你没这么多黄金。"
  set i_no_gold = "别闹，我没这么多黄金。"
  set tribute_of_gold = "黄金已提供。"
  set gold_received_thanks = "黄金已经收到。谢谢老板。"
  set you_no_lumber = "翻车了，你没这么多木材。"
  set i_no_lumber = "别闹，我没这么多木材。"
  set tribute_of_lumber = "木材已提供。"
  set lumber_received_thanks = "木材已经收到。谢谢老板。"
  set you_mistyped_tribute = "抱歉，你输入的数量有误。"
  set specified_player_not_valid = "指定的玩家无效。"
  set command_not_valid = "我无法识别该指令。"
  set all_not_allowed = "错误，不允许全选。"
  set player_id_mistyped = "错误，玩家ID错误。"
  set color_word_mistyped = "错误，玩家颜色或指令错误。"
  set not_specified_computer = "未指定玩家。"
  set next_page = "下一页"
  set previous_page = "上一页"
  set dlghdr_choose_ally = "选择盟友"
  set dlghdr_choose_number = "选择战术编号"
  set dlghdr_choose_player = "选择玩家"
  set dlghdr_game_type = "选择游戏模式"
  set dlghdr_root = "选择指令"
  set dlghdr_tribute_type = "选择资源类型"
  set dlghdr_tribute_amount = "选择资源数量"
  set dlgbutton_cancel = "取消"
  set dlgbutton_all = "选取全部AI（包括敌人）"
  set dlgbutton_gold = "选取的AI给我黄金"
  set dlgbutton_lumber = "选取的AI给我木材"
  set dlgbutton_give_gold = "给选取的AI黄金"
  set dlgbutton_give_lumber = "给选取的AI木材"
  set dlgbutton_tribute = "交换资源（仅盟友有效）"
  set dlgbutton_commander = "指挥模式（按ESC可指挥）"
  set dlgbutton_no_human = "AI内战（真人会自爆）"
  set dlgbutton_ai_only = "智能模式（按ESC只能暂停）"
  call SaveStr(translation, StringHash(language), StringHash("AI Setting"), "AI设置")
  call SaveStr(translation, StringHash(language), StringHash("Chatter"), "聊天")
  call SaveStr(translation, StringHash(language), StringHash("Language"), "更换语言")
  call SaveStr(translation, StringHash(language), StringHash("Chinese"), "简体中文")
  call SaveStr(translation, StringHash(language), StringHash("Language is now Chinese."), "语言变更为简体中文。")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Computer Chatter."), "电脑聊天已开启/已关闭")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Debug Mode."), "侦察已开启/关闭")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Balancing Mode."), "平衡模式已开启/已关闭")
  ...
  call SaveStr(translation, StringHash(language), StringHash("Showing all strategy names."), "显示所有战术")
  call SaveStr(translation, StringHash(language), StringHash("Strategy names"), "战术名称")
  call SaveStr(translation, StringHash(language), StringHash("Change Strategy"), "变更战术")
  call SaveStr(translation, StringHash(language), StringHash("Changes current strategy."), "改变战术。")
  call SaveStr(translation, StringHash(language), StringHash("Cancel Orders"), "取消命令")
  call SaveStr(translation, StringHash(language), StringHash("Stop"), "停止进攻")
  call SaveStr(translation, StringHash(language), StringHash("All Orders"), "全部命令")
  call SaveStr(translation, StringHash(language), StringHash("I have Canceled All commands you have given me so far."), "我已取消你安排的全部命令。")
  call SaveStr(translation, StringHash(language), StringHash("I do not know that command."), "我不能识别那个命令。")
  call SaveStr(translation, StringHash(language), StringHash("You have not specified a valid player."), "你尚未指定有效的玩家。")
  call SaveStr(translation, StringHash(language), StringHash("You have not specified a computer."), "你尚未指定有效的AI玩家。")
  call SaveStr(translation, StringHash(language), StringHash("Attack"), "攻击/防守")
  call SaveStr(translation, StringHash(language), StringHash("Starting Commander Thread Loop"), "启动命令循环程序")
  call SaveStr(translation, StringHash(language), StringHash("Receiving command"), "接收命令")
  call SaveStr(translation, StringHash(language), StringHash("Attack Orders"), "攻击命令")
  call SaveStr(translation, StringHash(language), StringHash("I have Canceled all Attack commands you have given me so far."), "我已取消你安排的全部攻击命令。")
  call SaveStr(translation, StringHash(language), StringHash("Build Orders"), "建造命令")
  call SaveStr(translation, StringHash(language), StringHash("I have Canceled all Build commands you have given me so far."), "我已取消你安排的全部建设命令。")
  call SaveStr(translation, StringHash(language), StringHash("Queue"), "行动控制")
  call SaveStr(translation, StringHash(language), StringHash("I have Canceled all Queued commands you have given me so far."), "我已取消你安排的全部下一步命令。")
  call SaveStr(translation, StringHash(language), StringHash("I will Stop any current Attack."), "我将停止全部进攻。")
  call SaveStr(translation, StringHash(language), StringHash("I will not Attack Anything."), "我不会攻击。")
  call SaveStr(translation, StringHash(language), StringHash("I will not Attack Creeps."), "我不会攻击野怪。")
  call SaveStr(translation, StringHash(language), StringHash("I will not Attack Enemies."), "我不会攻击敌人。")
  call SaveStr(translation, StringHash(language), StringHash("I will build Air2Air units now."), "我将训练空对空单位。")
  ...
  call SaveStr(translation, StringHash(language), StringHash("I will build farms now."), "我要造人口。")
  call SaveStr(translation, StringHash(language), StringHash("I will build towers now."), "我要造塔。")
  ...
  call SaveStr(translation, StringHash(language), StringHash("I have Started or Restarted your Queued commands."), "我已布置或重新布置你安排的下一步行动。")
  call SaveStr(translation, StringHash(language), StringHash("I have Paused your Queued commands."), "我已暂停你安排的下一步行动。")
  call SaveStr(translation, StringHash(language), StringHash("I have Unpaused your Queued commands."), "我已开启你安排的下一步行动。")
  call SaveStr(translation, StringHash(language), StringHash("I have Removed your last Queued command."), "我已取消你最后安排的下一步行动。")
  call SaveStr(translation, StringHash(language), StringHash("I have Queued the Attack/Guard location of current screen position."), "我已将进攻/防守当前位置设为下一步行动。")
  call SaveStr(translation, StringHash(language), StringHash("I have Queued the currently selected unit to Attack/Guard."), "我已将选择当前单位设为下一步行动。")
  call SaveStr(translation, StringHash(language), StringHash("Turned On/Off Automatic Strategy Change."), "自动战术判断已开启/已关闭")
  call SaveStr(translation, StringHash(language), StringHash("Restrict"), "限制行动")
  call SaveStr(translation, StringHash(language), StringHash("No Attacks"), "不攻击")
  call SaveStr(translation, StringHash(language), StringHash("No Creep Attacks"), "不攻击野怪")
  call SaveStr(translation, StringHash(language), StringHash("No Enemy Attacks"), "不攻击敌人")
  call SaveStr(translation, StringHash(language), StringHash("Farms"), "人口建筑")
  call SaveStr(translation, StringHash(language), StringHash("Towers"), "防御塔")
  call SaveStr(translation, StringHash(language), StringHash("Current screen"), "当前地点")
  call SaveStr(translation, StringHash(language), StringHash("Selected unit"), "选择的单位")
  call SaveStr(translation, StringHash(language), StringHash("Towerrush (Orc only"), "防御塔快攻（仅兽族可用）")
  call SaveStr(translation, StringHash(language), StringHash("Enemy"), "敌人")
  call SaveStr(translation, StringHash(language), StringHash("Start/Restart"), "开始/重新开始行动")
  call SaveStr(translation, StringHash(language), StringHash("No Strategy Change"), "关闭动态战术")
  call SaveStr(translation, StringHash(language), StringHash("Build"), "建造")
  call SaveStr(translation, StringHash(language), StringHash("Attack Here"), "攻击行动")
  call SaveStr(translation, StringHash(language), StringHash("Pause"), "暂停行动")
  call SaveStr(translation, StringHash(language), StringHash("Unpause"), "继续行动")
  call SaveStr(translation, StringHash(language), StringHash("Undo"), "取消行动")
  call SaveStr(translation, StringHash(language), StringHash("attack select"), "攻击选择单位行动")
  call SaveStr(translation, StringHash(language), StringHash("Air vs. Air"), "空对空单位")
  call SaveStr(translation, StringHash(language), StringHash("Air vs. Ground"), "空对地单位")
  call SaveStr(translation, StringHash(language), StringHash("Ground vs. Air"), "地对空单位")
  call SaveStr(translation, StringHash(language), StringHash("Ground vs. Ground"), "地对地单位")
endfunction
